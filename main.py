...
def _load_seo_section(logger):
    ...
    Devuelve None si el archivo no existe (deja la seccion vacia sin
    pisar datos previos).
    """
    repo_root = Path(__file__).resolve().parent

    # Refresh local rank snapshot (idempotent, only fires once/day)
    try:
    ...
    if not payload_path.exists():
        logger.warning(f"SEO payload not found at {payload_path} - skipping section")
        return None
    ...
    try:
        ...
        logger.warning(f"Error reading SEO payload: {e}")
        return None

    # Si hay un evolution.json local mas reciente (rank tracker en CI), usarlo
    local_evo = repo_root / "seo" / "evolution.json"
    if local_evo.exists():
        try:
            with open(local_evo, encoding="utf-8") as f:
                seo["rankings_evolution"] = json.load(f)
        except Exception as e:
            logger.warning(f"Error reading local evolution.json: {e}")

    summary = seo.get("audit_summary", {})
    logger.info(
        f"SEO section loaded from payload: avg score {summary.get('avg_score', '?')}/100, "
        f"{summary.get('total_articles', 0)} articulos"
    )
    return seo
